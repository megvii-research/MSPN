...
__global__ void batch_norm_collect_grad_statistics_kernel(
    ...
    */
    if (threadIdx.x == 0) {
        grad_exs[plane] = static_cast<scalar_t>(dot_p * weight_val * (-0.5) * pow(invstd, 3));
        grad_ex[plane] = static_cast<scalar_t>(grad_output_sum * weight_val * (-1.0) * invstd + \
                dot_p * weight_val * pow(invstd, 3) * mean);
    }
...
std::tuple<at::Tensor,at::Tensor,at::Tensor,at::Tensor> batch_norm_collect_grad_statistics_cuda(
    ...
    THCudaCheck(cudaGetLastError());
    return std::make_tuple(grad_weight, grad_bias, grad_ex, grad_exs);
}
